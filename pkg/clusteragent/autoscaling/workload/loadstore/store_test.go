...
func createSeriesPayload(i int, time_delta int64) *gogen.MetricPayload {
		...
		Series: []*gogen.MetricPayload_MetricSeries{
			{
				...
				Points: []*gogen.MetricPayload_MetricPoint{
					{
						Timestamp: time.Now().Unix() - time_delta, // time_delta seconds ago
						Value:     float64(i),
					},
				},
				Tags: []string{containerID, displayContainerName, namespace, deployment, kubeOwnerrefName, podName, containerName},
...
func createSeriesPayload2(i int, time_delta int64) *gogen.MetricPayload {
	containerID := fmt.Sprintf("container_id:%d", i)
	containerName := "container_name:container_test"
	displayContainerName := fmt.Sprintf("display_container_name:pod_%d-container_test", i)
	namespace := "kube_namespace:test"
	deployment := "kube_deployment:nginx_test"
	kubeOwnerrefName := "kube_ownerref_name:nginx_test"
...
func TestStoreAndPurgeEntities(t *testing.T) {
	...
	for _, time_delta := range []int64{100, 85, 80} {
		for i := 0; i < numPayloads; i++ {
			payload := createSeriesPayload(i, time_delta)
			entities := createEntitiesFromPayload(payload)
			store.SetEntitiesValues(entities)
			payload2 := createSeriesPayload2(i, time_delta)
