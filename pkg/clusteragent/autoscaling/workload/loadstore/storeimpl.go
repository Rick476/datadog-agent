...
func (es *EntityStore) deleteInternal(hash uint64) {
	if toBeDelItem, exists := es.key2ValuesMap[hash]; exists {
		compositeKeyHash := generateCompositeKey(toBeDelItem.entity.Namespace, toBeDelItem.entity.PodOwnerName, toBeDelItem.entity.MetricName)
		if _, ok := es.keyAttrTable[compositeKeyHash]; ok {
			if dataPerPod, ok := (es.keyAttrTable[compositeKeyHash].pods)[toBeDelItem.entity.PodName]; ok {
				// Delete the entity from the keyAttrTable
				if toBeDelItem.entity.EntityType == ContainerType {
					delete(dataPerPod.containers, toBeDelItem.entity.EntityName)
				}
